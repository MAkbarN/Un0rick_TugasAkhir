...
class us_json:
    ...
    def mkImg(self):
        ...
        if self.processed: #@todo check this to get env & al
            ...
            plt.tight_layout()
            file_name = "images/"+"Uji"+self.iD+"-"+str(self.N)+".png"
            plt.savefig(file_name)
    ...
    def save_csv(self):
        """
           Saves the dataset as an .csv, in the data folder.
        """
        path_csv = "data/"+self.iD+"-"+str(self.N)+".csv" # @todo: create folder if not.
        np.savetxt(path_csv, self) 
    ...
    def plot_detail(self, nb_line, Start, Stop): #@todo: use it when processing data
        ...
        #data = np.asarray(plot_time_series,plot_signal)
        #path_csv = "data/"+self.iD+"-"+str(self.N)+".csv" # @todo: create folder if not.
        #np.savetxt(path_csv,data) 
...
if __name__ == "__main__":
    print("Welcome to Un0rick board!")
    print("Modified by: Akbar")
    print("Loaded!")
        ...
        if "process" in sys.argv[1]:
            ...
            for MyDataFile in os.listdir("./data/"):
                if MyDataFile.endswith(".json"):
                    ...
                    # Penamaan urutan file
                    print("File ke berapa:")
                    num = int(input())
                    y.JSONprocessing("./data/"+MyDataFile)
                    y.create_fft() 
                    
                    # Akses array data untuk tiap tipe sinyal (mentah,terfilter,envelope)
                    time = y.t[0:y.len_line]
                    raw_signal = y.tmp[0:y.len_line]
                    filtered_signal = y.filtered_signal[0:y.len_line]
                    envelope_signal = y.EnvHil[0:y.len_line]
                    
                    # Simpan file dengan tipe data csv
                    file_csv_raw = "./data/" +str(num)+"_sinyal_raw.csv"
                    file_csv_filtered = "./data/" +str(num)+"_sinyal_filtered.csv"
                    file_csv_env = "./data/" +str(num)+"_sinyal_envelope.csv"
                    
                    data_csv_raw = np.array([time, raw_signal])
                    data_csv_filtered = np.array([time, filtered_signal])
                    data_csv_env = np.array([time, envelope_signal])
                    
                    data_csv_raw = data_csv_raw.transpose()
                    data_csv_filtered = data_csv_filtered.transpose()
                    data_csv_env = data_csv_env.transpose()
                    
                    np.savetxt(file_csv_raw, data_csv_raw, delimiter=',')
                    np.savetxt(file_csv_filtered, data_csv_filtered, delimiter=',')
                    np.savetxt(file_csv_env, data_csv_env, delimiter=',')
                    
                    print("ukuran data:",np.size(y))

                    # Simpan file, tipe data json
                    y.save_npz()
                    y.mkImg()
                    #if y.Nacq > 1:
                    #    y.mk2DArray()
